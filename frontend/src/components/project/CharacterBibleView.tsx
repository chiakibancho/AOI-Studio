      ...
      <div className="flex items-center gap-2">
        {canGenerate && (
          <Button
            ...
            disabled={isBusy}
          >
            {character.status === 'draft' ? 'Generate' : '再生成する'}
          </Button>
        )}
        ...
        >
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="FLUXにそのまま渡すプロンプトを全文で入力してください"
              className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text-primary"
            />
          </div>
              ...
              <div className="px-3 pb-3 flex flex-col gap-2">
                ...
                {(
                  ...
                  <div key={field} className="flex items-center gap-2">
                    <label className="w-20 flex-shrink-0 text-xs font-medium text-text-secondary">
                      {label}
                    </label>
                    <input
                      type="text"
                      value={advanced[field]}
                      onChange={(e) => handleAdvancedChange(field, e.target.value)}
                      aria-label={label}
